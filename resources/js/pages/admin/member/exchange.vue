...
import { EXCHANGE_STATUS } from '@/utils/statusCodes'
...
const fetchData = async () => {
  try {
    var response = await axios.get('/admin/exchange?action=getData', {
      params: {
        q: search.value || null,
          ...
          <VDataTableServer
            ...
            <template #item.username="{ item }">
              <div class="d-flex flex-column">
                <span class="text-primary font-weight-medium">{{ item.member.username }}</span>
              </div>
            </template>
            ...
            <template #item.point="{ item }">
              <div class="d-flex flex-column">
                <span class="text-uppercase">{{ $formatNumber(item.point) }}</span>
              </div>
            </template>
            <template #item.status="{ item }">
              <VChip
                :color="EXCHANGE_STATUS.color(item.status)"
                label
                size="x-small"
              >
                {{ EXCHANGE_STATUS.label(item.status) }}
              </VChip>
            </template>

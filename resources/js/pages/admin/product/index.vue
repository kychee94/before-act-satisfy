...
import { PRODUCT_STATUS } from '@/utils/statusCodes'
...
const res = ref(JSON.parse(JSON.stringify(props.products)))

// Fetch data
const fetchData = async () => {
  try {
    var response = await axios.get('/admin/product?type=getData', {
      params: {
        q: search.value || null,
...
const performBulkAction = async () => {
  ...
  try {
    var response = await axios.post('/admin/product/delete', {
        uuids: ids,
    })
        ...
        <VCardText>
            ...
            <template #prepend>
              <div class="flex flex-wrap gap-1 w-full">
                ...
                <div class="basis-full sm:basis-auto sm:w-[200px]">
                  <AppSelect
                    v-model="category"
                    :items="[{ id: '', category_name: $t('Select All') }, ...props.categories]"
            ...
            <template #append>
              ...
              <div v-else>
                ...
                <VBtn
                  ...
                  @click="$inertia.visit('/admin/product/create')"
                >
                  {{$t('Add Products')}}
                </VBtn>
              </div>
          ...
          <VDataTableServer
            ...
            <template #item.uuid="{ item }">
              <div class="flex items-center">
                <VImg 
                  ...
                  max-width="50"
                  aspect-ratio="1"
                  class="cursor-pointer rounded me-2"
                  @click="openLightbox(item.image)"
                />
                <span class="text-primary font-weight-medium text-wrap">{{ item.name }}</span>
              </div>
            </template>
            ...
            <template #item.status="{ item }">
              <VChip
                :color="PRODUCT_STATUS.color(item.status)"
                label
                size="x-small"
              >
                {{ PRODUCT_STATUS.label(item.status) }}
              </VChip>
            </template>
              ...
              <VTooltip :text="$t('View')" location="bottom">
                <template #activator="{ props }">
                  <VBtn
                      ...
                      class="mx-1"
                      icon
                      @click="$inertia.visit('/admin/product/edit/'+ item.uuid)"
                    >

...
const res = ref(JSON.parse(JSON.stringify(props.brands)))

// Fetch data
const fetchData = async () => {
  try {
    var response = await axios.get('/admin/brand?type=getData', {
      params: {
        q: search.value || null,
    ...
    res.value = response.data.brands
...
const performBulkAction = async () => {
  ...
  try {
    var response = await axios.post('/admin/brand/delete', {
        uuids: ids,
    })
        ...
        <VCardText>
            ...
            <template #append>
              ...
              <div v-else>
                ...
                <VBtn
                  ...
                  @click="$inertia.visit('/admin/brand/create')"
                >
                  {{$t('Add Brand')}}
                </VBtn>
              </div>
          ...
          <VDataTableServer
            ...
            <template #item.status="{ item }">
              <VChip
                :color="BRAND_STATUS.color(item.status)"
                label
                size="x-small"
              >
                {{ BRAND_STATUS.label(item.status) }}
              </VChip>
            </template>
              ...
              <VTooltip :text="$t('View')" location="bottom">
                <template #activator="{ props }">
                  <VBtn
                      ...
                      class="mx-1"
                      icon
                      @click="$inertia.visit('/admin/brand/edit/'+ item.uuid)"
                    >

...
import { AGENT_STATUS } from '@/utils/statusCodes'
...
const res = ref(JSON.parse(JSON.stringify(props.agents)))

// Fetch data
const fetchData = async () => {
  try {
    var response = await axios.get('/admin/agent?type=getData', {
      params: {
        q: search.value || null,
    ...
    res.value = response.data.agents
...
const performBulkAction = async () => {
  ...
  try {
    var response = await axios.post('/admin/agent/delete', {
        uuids: ids,
    })
          ...
          <VCardItem class="px-0 d-flex flex-wrap justify-space-between gap-4">
            ...
            <template #append>
              ...
              <VBtn
                ...
                @click="$inertia.visit('/admin/agent/create')"
              >
                {{$t('Add Agent')}}
              </VBtn>
            </template>
          ...
          <VDataTableServer
            ...
            <template #item.uuid="{ item }">
              <div class="d-flex flex-column">
                ...
                <span class="text-caption text-grey-darken-1">{{ item.name }}</span>
              </div>
            </template>
            ...
            <template #item.status="{ item }">
              <VChip
                :color="AGENT_STATUS.color(item.status)"
                label
                size="x-small"
              >
                {{ AGENT_STATUS.label(item.status) }}
              </VChip>
            </template>
              ...
              <VTooltip :text="$t('View')" location="bottom">
                <template #activator="{ props }">
                  <VBtn
                      ...
                      class="mx-1"
                      icon
                      @click="$inertia.visit('/admin/agent/edit/'+ item.uuid)"
                    >

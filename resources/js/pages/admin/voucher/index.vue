...
import { VOUCHER_STATUS } from '@/utils/statusCodes'
...
const res = ref(JSON.parse(JSON.stringify(props.vouchers)))

// Fetch data
const fetchData = async () => {
  try {
    var response = await axios.get('/admin/voucher?type=getData', {
      params: {
        q: search.value || null,
    ...
    res.value = response.data.vouchers
...
const performBulkAction = async () => {
  ...
  try {
    var response = await axios.post('/admin/voucher/delete', {
        uuids: ids,
    })
        ...
        <VCardText>
            ...
            <template #prepend>
              <div class="flex flex-wrap gap-1 w-full">
                ...
                <div class="basis-full sm:basis-auto sm:w-[200px]">
                  <AppSelect
                    v-model="type"
                    :items="[{ key: '', value: $t('Select All') }, ...props.types]"
                    item-title="value"
                    item-value="key"
                    :placeholder="$t('Type')"
                  />
            ...
            <template #append>
              ...
              <div v-else>
                <VBtn
                  ...
                  @click="$inertia.visit('/admin/voucher/create')"
                >
                  {{$t('Add %s', {'name': $t('Voucher')})}}
                </VBtn>
              </div>
            </template>
          </VCardItem>
          ...
          <VDataTableServer
            ...
            <template #item.status="{ item }">
              <VChip
                :color="VOUCHER_STATUS.color(item.status)"
                label
                size="x-small"
              >
                {{ VOUCHER_STATUS.label(item.status) }}
              </VChip>
            </template>
              ...
              <VTooltip :text="$t('View')" location="bottom">
                <template #activator="{ props }">
                  <VBtn
                      ...
                      class="mx-1"
                      icon
                      @click="$inertia.visit('/admin/voucher/edit/'+ item.uuid)"
                    >

...
languages.forEach(lang => {
  imageInput.pc[lang.lang_code] = 
    JSON.parse(props.slideshow?.slideshow_image)?.pc?.[lang.lang_code] || ''
  imageInput.mobile[lang.lang_code] = 
    JSON.parse(props.slideshow?.slideshow_image)?.mobile?.[lang.lang_code] || ''
})

const activeTab = ref(languages[0].lang_code)
const formRef = ref()
const form = useForm({
  uuid: props.slideshow.uuid,
  slideshow_title: props.slideshow.slideshow_title || '',
  slideshow_link: props.slideshow.slideshow_link || '',
  image: imageInput,
  imageInput: imageInput,
  slideshow_status: Boolean(props.slideshow.slideshow_link) || 0
})
...
const onSubmit = (formRef, formInstance) => {
  formInstance.transform(data => ({
    ...formInstance.data(),
  })).post(route('admin.setting.slideshow.update'), {
      forceFormData: true,
      onStart: () => {
        loadingStore.start()
      },
    ...
    <VForm
            ...
            <VCard
              ...
              <VCardText>
                <VRow>
                    ...
                    <VCol cols="12">
                      <AppTextField
                        v-model="form.slideshow_link"
                        :label="t('Slideshow Link')"
                      />
                    </VCol>
                </VRow>
              </VCardText>
      ...
      <div class="d-flex justify-end gap-x-4">
        <VBtn
          ...
          variant="text"
          class="mt-4"
          @click="$inertia.visit('/admin/setting/slideshow')"
          >
          <VIcon start>
            mdi-arrow-left
          </VIcon>

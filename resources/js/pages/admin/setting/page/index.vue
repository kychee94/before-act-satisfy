...
import { PAGE_STATUS } from '@/utils/statusCodes'
...
const fetchData = async () => {
  try {
    var response = await axios.get('/admin/setting/page?type=getData', {
      params: {
        q: search.value || null,
...
const deleteItem = async uuid => {
  ...
  try {
    var response = await axios.post('/admin/setting/page/delete', {
        uuid: uuid,
    })
        ...
        <VCardText>
          <VCardItem class="px-0 d-flex flex-wrap justify-space-between gap-4">
            ...
            <template #append>
              <VBtn
                ...
                @click="$inertia.visit('/admin/setting/page/create')"
              >
                {{$t('Add Page')}}
              </VBtn>
            </template>
          ...
          <VDataTableServer
            ...
            <template #item.status="{ item }">
              <VChip
                :color="PAGE_STATUS.color(item.status)"
                label
                size="x-small"
              >
                {{ PAGE_STATUS.label(item.status) }}
              </VChip>
            </template>
              ...
              <VTooltip :text="$t('View')" location="bottom">
                <template #activator="{ props }">
                  <VBtn
                      ...
                      class="mx-1"
                      icon
                      @click="$inertia.visit('/admin/setting/page/edit/'+ item.uuid)"
                    >

...
const res = ref(JSON.parse(JSON.stringify(props.spins)))
...
const fetchData = async () => {
  try {
    var response = await axios.get('/admin/spin?type=getData', {
      params: {
        q: search.value || null,
...
const getFormattedStyle = (style) => {
  if (!style || style == 'null' || style === null || style === undefined) return ''
  ...
  try {
    ...
    // If parsing fails, try to clean escaped quotes manually
    if (typeof style === 'string') {
...
const updateStyle = (value, index) => {

  try {
    // Try to parse as JSON
    const parsed = JSON.parse(value)
    records.value[index].style = parsed
  } catch (e) {
    // If it's not valid JSON, store as string
    records.value[index].style = value
  }
}
...
const saveSegment = async () => {
  ...
  try {
    ...
    records.value.forEach((record, index) => {
      ...
      // Stringify style if it's an object
      const styleValue = typeof record.style === 'object' 
        ? JSON.stringify(record.style) 
        : (record.style || '')

      formData.append(`records[${index}][style]`, styleValue)
    ...
    var response = await axios.post('/admin/spin/update', formData, {
      headers: {
              ...
              <tr
                ...
                <td class="text-center">
                  <VImg
                    ...
                    width="100"
                    height="100"
                    @click="openLightbox(spin.imageUrl)"
                  />
                  <input
                    type="file"
                ...
                <td>
                  <AppTextarea
                    :model-value="getFormattedStyle(spin.style)"
                    @update:model-value="updateStyle($event, index)"
                    variant="outlined"
                    density="compact"
                  />
                </td>
                <td>
                  <VBtn icon="mdi-delete" color="error" variant="text" @click="deleteItem(spin.uuid)" />
                </td>
              </tr>
            </tbody>
          </VTable>
